...
toList t = case t of
  Leaf x -> [x]
  Node _ l r -> toList l ++ toList r

-- Fetches the k-th element in the tree
fetch :: Int -> Tree a -> a
fetch k t = case t of
  Leaf x -> x
  Node _ l r -> if k < size l then fetch k l else fetch (k - size l) r

-- Updates the k-th element of a tree
update :: Show a => Int -> a -> Tree a -> Tree a
update k x t = case t of
  Leaf _ -> Leaf x
  Node _ l r -> if k < size l then node (update k x l) r else node l (update (k - size l) x r)
...
wellFormed t = case t of
  Leaf _ -> True
  Node n l r -> size l == size r && n == size l + size r && wellFormed l && wellFormed r
